...
router.post('/schedule-readiness', authenticateToken, async (req, res) => {
  try {
    ...
    const context = String(req.body?.context || 'Assess a proposed appointment schedule for conflicts, travel buffers, and attendance risks.').slice(0, 4000);
    const response = await fetch(`${baseUrl}/chat/completions`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ model, messages: [
        { role: 'system', content: 'You are a scheduling operations assistant. Give a concise readiness assessment with conflicts, evidence gaps, and practical next actions.' },
    ...
    const stored = await pool.query(
      `INSERT INTO runtime_ai_results (user_id, tenant_id, feature, input, content, model, provider_receipt)
       VALUES ($1, $2, 'schedule-readiness', $3, $4, $5, $6) RETURNING id, created_at`,
    ...
    console.error('[runtime-ai] schedule-readiness failed:', error.message);
    res.status(502).json({ error: 'OpenRouter schedule readiness failed' });
  }
});
